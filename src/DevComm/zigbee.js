...
module.exports = class Zigbee extends EventEmitter {
  ...
  async connectSerial() {
    try {
      ...
      console.log("error connectSerial :", e);
      throw e;
    }
  }
  ...
  async setPort() {
    try {
      ...
      this.Serial.pipe(this.xbee.parser);
      this.xbee.builder.pipe(this.Serial);
      return;
    } catch (e) {
      throw e;
    }
  ...
  eventsSerial() {
    this.xbee.parser.on("data", frame => {
      console.log("data", frame.data.toString());
    ...
    this.Serial.on("error", error => {
      error.disconnected ? this.setConnect(false) : null;
    });
  }
  ...
  fake() {
    ...
    setInterval(() => {
      const frame = `03,3316,1.2,${moment().format(
        "DD/MM/YYYY"
